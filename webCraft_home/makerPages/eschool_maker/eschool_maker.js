...
function isStepValid() {
    ...
    if (currentStep === 0) {
        ...
        if (!label1 || label1.innerText === "Select School Type") {
        ...
        if (!label2 || label2.innerText === "Select Student Type") {
...
async function collectAllData() {
    ...
    if (label1 && label1.innerText !== "Select School Type") data['form-scType'] = label1.innerText;
    if (label2 && label2.innerText !== "Select Student Type") data['form-sType'] = label2.innerText;
    ...
    // Collect school members (testimonials)
    data.schoolMembers = [];
    const memberCards = document.querySelectorAll('.school-card');
    memberCards.forEach((card, i) => {
        const preview = document.getElementById(`img-preview-${i}`);
        data.schoolMembers.push({
        ...
        return `
            <div class="site-card">
                ...
                <div class="card-footer" style="display: flex; gap: 10px; padding: 15px;">
                    <button class="btn btn-outline black" onclick="handleOpenModal(${p.id})" style="color: var(--text)">Edit</button>
...
async function launchProject(id) {
    ...
    const previewUrl = 'https://github.com/binizab/Web_Craft-Project/create_websites/e-school/eschool_home/eschool_home.html';
...
async function handleOpenModal(editId = null) {
    ...
    const defaultLabels = [
        { id: 'label1', text: "Select School Type" },
        { id: 'label2', text: "Select Student Type" },
...
if (p.images && Array.isArray(p.images)) {
    ...
    p.images.forEach((imgData, index) => {
        ...
        if (imgEl && imgData.src && imgData.src.length > 10) {
            ...
            // Restore school members (testimonials)
            if (p.schoolMembers && p.schoolMembers.length > 0) {
                const countInput = document.getElementById('cardCount');
                if (countInput) {
                    countInput.value = p.schoolMembers.length;
                    updateSwiper();
                }
                
                setTimeout(() => {
                    p.schoolMembers.forEach((member, i) => {
                        const preview = document.getElementById(`img-preview-${i}`);
    ...
    card.innerHTML = `
        ...
        <div class="custom-dropdown" id="grade-drop-${id}">
            ...
            <div class="dropdown-content">
                ${['Pre-Primary', 'Elementary', 'Middle', 'High School', 'University', 'College'].map(g => `
...
function updateSwiper() {
    ...
    for (let i = 0; i < count; i++) {
        ...
        slide.innerHTML = `
            <div class="school-card upload-card" id="card-${i}">
...
function toggleTheme() {
    ...
    // Dynamically flip the font-awesome icon structure
    const toggleIcon = document.querySelector('#themeToggle i');
    if (toggleIcon) {
        if (isLight) {
            toggleIcon.classList.remove('fa-sun');
...
// Make toggleTheme accessible globally globally
window.toggleTheme = toggleTheme;

// Trigger theme checking immediately on load initialization 
